...
// Devices for one owner (by email). With no email, every device - used only
// by the admin path.
async function listSubscriptions(email) {
  const q = email ? "&email=ilike." + encodeURIComponent(email) : "";
  return (await supabase("push_subscriptions?select=id,endpoint&limit=50" + q)) || [];
}
...
async function notifyDevices(ownerEmail) {
  if (!pushConfigured()) return { skipped: "push not configured" };

  let subs;
  try {
    // Owner's devices, plus the admin's so the founder still sees everything.
    const admin = (process.env.PORTAL_EMAIL || "").toLowerCase();
    const emails = {};
    if (ownerEmail) emails[String(ownerEmail).toLowerCase()] = 1;
    if (admin) emails[admin] = 1;
    const lists = await Promise.all(Object.keys(emails).map(listSubscriptions));
    const seen = {};
    subs = [].concat.apply([], lists).filter(function (s) {
      if (seen[s.endpoint]) return false; seen[s.endpoint] = 1; return true;
    });
  } catch (err) {
